  ...
  if (type === "bubbles") {
    canvas.addEventListener("pointerdown", (event) => {
      ...
      for (let i = surface.bubbles.length - 1; i >= 0; i -= 1) {
        ...
        const hitDistance = (dx / (radius * bubble.widthRatio)) ** 2 + (dy / radius) ** 2;
        if (hitDistance <= 1) {
          hit = i;
          break;
        }
...
function drawBubbles(surface, now, dt) {
  ...
  surface.bubbles.forEach((bubble) => {
    ...
    const riseRate = 0.55 + settings.motion / 220;
    bubble.y -= bubble.rise * riseRate * seconds;
  ...
  surface.particles = liveParticles;
  ctx.shadowBlur = 0;
...
function frame(now) {
  ...
  try {
    ...
    paused = true;
    console.error("Stim Yard animation paused after a drawing error:", error);
    document.querySelectorAll('[data-action="pause"]').forEach((button) => {
      button.textContent = "Resume";
      button.setAttribute("aria-pressed", "true");
    });
    updateStatus("The animation paused after a drawing issue. Press Reset or choose another game to resume.");
  }
