...
function library() {
  ...
  main.querySelectorAll("[data-quarter]").forEach(
    (b) =>
      (b.onclick = () => {
        s.quarter = b.dataset.quarter;
        library();
      }),
  );
  ["search", "format", "topic"].forEach((id, i) =>
    document
      .getElementById(id)
      .addEventListener(id === "search" ? "input" : "change", (e) => {
        s[["query", "type", "topic"][i]] = e.target.value;
        libraryResults();
      }),
  );
  libraryResults();
...
function libraryResults() {
  ...
  const content = posts.filter(
    (p) =>
      p.destinations?.includes("library") &&
      (!s.type || p.type === s.type) &&
      (!s.topic || p.topics?.includes(s.topic)) &&
      (!query ||
        [p.title, p.excerpt, ...(p.topics || [])]
          .join(" ")
          .toLowerCase()
          .includes(query)) &&
      (!s.quarter ||
        (p.weeks || []).some(
          (w) =>
            curriculum.weeks.find((x) => x.id === w)?.quarter === +s.quarter,
        ) ||
        (p.modules || []).some(
          (m) =>
            curriculum.modules.find((x) => x.id === m)?.quarter === +s.quarter,
        )),
  );
  const target = document.getElementById("library-results");
  if (s.view === "resources") {
    target.innerHTML = `<p class="fine" role="status">${content.length} published resource${content.length === 1 ? "" : "s"}</p><div class="grid">${content.map(card).join("") || empty("No published resources match these filters. New videos and writing will appear here when added.")}</div>`;
    return;
  }
  const linked = (w) => content.filter((p) => p.weeks?.includes(w.id));
  const visible = (w) =>
    (!query && !s.type && !s.topic) ||
    linked(w).length ||
    content.some((p) => p.modules?.includes(w.module)) ||
    (!s.type &&
      !s.topic &&
      [w.title, curriculum.modules.find((m) => m.id === w.module)?.title || ""]
        .join(" ")
        .toLowerCase()
        .includes(query));
  const weekHTML = (w) => {
    const items = linked(w);
    return `<div class="week"><small>${String(w.id).padStart(2, "0")}</small><b>${esc(w.title)}</b><span class="badge ${items.length ? "" : "planned"}">${items.length ? items.length + " resource" + (items.length === 1 ? "" : "s") : "Planned"}</span>${items.length ? `<div class="week-content">${items.map((p) => `<a href="#post/${encodeURIComponent(p.id)}">${p.type === "video" ? "▷" : "↳"} ${esc(p.title)} <small>· ${typeLabel[p.type]}</small></a>`).join("")}</div>` : ""}</div>`;
  };
  const quarterHTML = curriculum.quarters
    .filter((q) => !s.quarter || +s.quarter === q.id)
    .map((q) => {
      const weeks = curriculum.weeks.filter(
        (w) => w.quarter === q.id && visible(w),
      );
      if (!weeks.length) return "";
      return `<details class="quarter" ${s.quarter || query || s.type || s.topic || q.id === 1 ? "open" : ""}><summary><span class="qno">0${q.id}</span><div><small>QUARTER ${q.id} · ${esc(q.subtitle)}</small><h3>${q.name}</h3><small>${esc(q.description)}</small></div></summary>${curriculum.modules
        .filter((m) => m.quarter === q.id)
        .map((m) => {
          const mw = weeks.filter((w) => w.module === m.id);
          const extras = content.filter(
            (p) =>
              p.modules?.includes(m.id) &&
              !p.weeks?.some(
                (w) =>
                  curriculum.weeks.find((z) => z.id === w)?.module === m.id,
              ),
          );
          return !mw.length && !extras.length
            ? ""
            : `<section class="module"><p class="eyebrow">Module ${String(m.id).padStart(2, "0")}</p><h3>${esc(m.title)}</h3><p>${esc(m.question)}</p>${mw.map(weekHTML).join("")}${extras.map((p) => `<p><a href="#post/${encodeURIComponent(p.id)}">${esc(p.title)} →</a></p>`).join("")}</section>`;
        })
        .join("")}${weeks
        .filter((w) => !w.module)
        .map(
          (w) =>
            `<section class="module"><p class="eyebrow">${q.id === 4 ? "Final capstone" : "Quarterly synthesis lab"}</p>${weekHTML(w)}</section>`,
        )
        .join("")}</details>`;
    })
    .join("");
...
function blog() {
  main.innerHTML = `<div class="wrap">${heading("The blog", "Space for a longer thought.", "Original posts, reflections and ideas. Separate from the curriculum, organized by theme.")}<div class="filters"><label>Search posts<input id="blog-search" type="search" placeholder="Find a thought…"></label><label>Theme<select id="blog-topic"><option value="">All themes</option>${[
    ...new Set(
      posts.filter((p) => p.type === "blog").flatMap((p) => p.topics || []),
    ),
  ]
    .sort()
    ...
    .join(
      "",
    )}</select></label></div>${loadError ? `<p class="notice error">${esc(loadError)}</p>` : ""}<div class="grid" id="blog-posts"></div></div>`;
  const update = () => {
    ...
    document.getElementById("blog-posts").innerHTML =
      posts
        ...
        .join("") ||
      empty(
        "No blog posts here yet. Published posts will automatically appear under their themes.",
      );
  };
  document.getElementById("blog-search").oninput = update;
  document.getElementById("blog-topic").onchange = update;
  update();
}
function post(id) {
  ...
  if (play)
    play.onclick = () => {
      ...
      play.parentElement.replaceWith(iframe);
    };
}
function toolsPage() {
...
function compass() {
  ...
  const update = (forced) => {
    const near = nearestEmotions(emotions, x, y);
    selectedEmotion = forced || near[0];
    document
      .getElementById("marker")
      .setAttribute(
        "transform",
        `translate(${260 + x * 2.08} ${260 - y * 2.08})`,
      );
    document.getElementById("pleasantness").value = x;
    document.getElementById("energy").value = y;
    document.getElementById("x-out").value = `${x}`;
    document.getElementById("y-out").value = `${y}`;
    document
      .querySelectorAll(".dot")
      .forEach((d) =>
        d.classList.toggle(
          "selected",
          d.dataset.emotion === selectedEmotion.id,
        ),
      );
    document.getElementById("emotion-reading").innerHTML =
      `<h2>${esc(selectedEmotion.label)}</h2><p>${esc(selectedEmotion.family)}${selectedEmotion.parent ? " / " + esc(selectedEmotion.parent) : ""}</p><span class="badge">${forced ? "Your chosen word" : "Nearest starting point"}</span>`;
    document.getElementById("nearby").innerHTML = near
      .map(
        (e) =>
          `<button data-id="${e.id}" class="${selectedEmotion.id === e.id ? "active" : ""}">${esc(e.label)}</button>`,
      )
      .join("");
    document
  ...
  document.getElementById("pleasantness").oninput = (e) => {
    x = +e.target.value;
    update();
  };
  ...
  const move = (e) => {
    ...
    x = Math.max(-100, Math.min(100, Math.round((p.x - 260) / 2.08)));
    y = Math.max(-100, Math.min(100, Math.round((260 - p.y) / 2.08)));
    update();
  };
  svg.onpointerdown = (e) => {
    dragging = true;
  ...
  const vocab = () => {
    const query = document.getElementById("emotion-search").value.toLowerCase();
    document.getElementById("vocabulary").innerHTML =
      Object.keys(colours)
        .map((f) => {
          const words = emotions.filter(
            (e) =>
              e.family === f &&
          ...
          return words.length
            ? `<h3>${f}</h3><div class="emotion-list">${words.map((e) => `<button data-id="${e.id}" aria-label="${esc(e.label + " · " + e.family + (e.parent ? " · " + e.parent : ""))}">${esc(e.label)}</button>`).join("")}</div>`
            : "";
        })
        .join("") ||
      '<p class="notice">No wheel labels match. Try a related word, or explore the families above.</p>';
    document
      .getElementById("vocabulary")
      .querySelectorAll("button")
      .forEach(
        (b) =>
          (b.onclick = () => {
            select(emotions.find((e) => e.id === b.dataset.id));
            document
              .getElementById("emotion-reading")
              .scrollIntoView({ block: "center", behavior: "smooth" });
          }),
      );
  };
  document.getElementById("emotion-search").oninput = vocab;
  vocab();
  update(selectedEmotion);
}
const prompts = [
  "Describe what happened and what you directly noticed. You can distinguish an observation from something that feels true.",
  "What meaning, prediction or assumption have you added? What else could explain it?",
  "What might feel threatened or important: safety, dignity, connection, fairness, or something else? It is okay not to know.",
  "Name the values pulling in different directions. What does each one ask of you?",
  "Given this situation, which value deserves to guide the next step? Why?",
  "Choose one specific action small enough to take today.",
  "Consider both possibilities without judging yourself. Moving away from discomfort can sometimes be protective.",
...
function questioning() {
  ...
  main.querySelectorAll("[data-step]").forEach(
    (b) =>
      (b.onclick = () => {
        storeAnswer();
        questionStep = +b.dataset.step;
        questioning();
      }),
  );
  const input = document.getElementById("answer");
  if (input) {
    input.oninput = storeAnswer;
  ...
  const status = (t) =>
    (document.getElementById("reflection-status").textContent = t);
  document.getElementById("save-reflection").onclick = () => {
    try {
      ...
      status("This browser could not save the draft.");
    }
  };
  document.getElementById("load-reflection").onclick = () => {
    try {
      const d = JSON.parse(localStorage.getItem("ns-reflection"));
      if (!d || !Array.isArray(d.answers) || d.answers.length !== 8)
        return status("No saved draft found.");
      if (
        answers.some(Boolean) &&
        !confirm("Replace the current answers with your saved draft?")
      )
        return;
  ...
  document.getElementById("clear-reflection").onclick = () => {
    try {
      localStorage.removeItem("ns-reflection");
      status(
        "Saved draft removed. The answers currently in this tab are unchanged.",
      );
    } catch {
      status("Could not access saved drafts.");
    }
  };
}
...
function navigatorPage() {
  ...
  const paths = {
    overwhelmed: [
      ...
      [
        "Name the one pressure that is loudest.",
        "Choose one thing that can wait.",
        "Pick a manageable two-minute action.",
      ],
    ],
    stuck: [
      "Make the decision more visible",
      [
        "Write down the options you are considering.",
        "Name the value each option serves.",
        "Choose one small, reversible way to learn more.",
      ],
    ],
    disconnected: [
      "Reconnect with your surroundings",
      [
        "Notice three things you can see nearby.",
        "Choose a comfortable physical anchor, such as your feet on the floor.",
        "Consider one person or familiar activity you could reconnect with.",
      ],
    ],
  };
  const show = (key) => {
...
function route() {
  ...
  (
    ({
      ...
      about,
      install: () => renderInstall(main),
      community: () => renderCommunity(main, "feedback"),
      ...
      post: () => post(decodeURIComponent(id || "")),
...
try {
  ...
  if (!Array.isArray(data.posts)) throw Error();
  posts = data.posts
    .filter((p) => p.status === "published")
    .sort((a, b) => String(b.publishedAt).localeCompare(String(a.publishedAt)));
} catch {
  loadError =
    "Published content could not be loaded. Check your connection and refresh; the curriculum and tools remain available.";
}
